<template>
  <div>
    <v-row no-gutters>
      <v-col cols="12" sm="4">
        <v-card flat>
          <v-card-title>Signatory Form </v-card-title>
          <v-card-text>
            <v-form ref="form">
              <v-text-field
...
export default {
  ...
  methods: {
    ...
    createSignatory() {
      ...
      axios
        .post(route("signatories.store"), this.formData)
        .then((res) => {
          this.submit = false;
          this.getSignatories();
          this.resetForm();
    ...
    updateSignatory() {
      ...
      axios
        .post(route("signatories.update", this.formData.id), this.formData)
        .then((res) => {
          this.submit = false;
          this.getSignatories();
          this.resetForm();
    ...
    deleteSignatory(signatory) {
      if (
        confirm(
          `Are you sure you want to delete ${signatory.name} - ${signatory.position} ?`
        )
      ) {
        axios
          .delete(route("signatories.destroy", signatory.id))
          .then((res) => {
            this.getSignatories();
            this.resetForm();
          })
