    ...
    <v-row no-gutters>
      <v-col cols="12" sm="4">
        <v-form ref="form_settings">
          <v-text-field
            v-model="formData.min_range"
            label="Min Range"
            :error-messages="formErrors.min_range"
          ></v-text-field>

          <v-text-field
            v-model="formData.max_range"
            label="Max Range"
            :error-messages="formErrors.max_range"
          ></v-text-field>
          ...
          <v-autocomplete
            ...
            clearable
            dense
            v-model="formData.signatories"
            :loading="!signatories"
            label="For Signatory"
            :items="signatories"
            deletable-chips
            :error-messages="formErrors.signatories"
            item-value="id"
            item-text="name"
            hide-details="auto"
          >
          ...
          <v-card-text>
            ...
            >
              <template v-slot:item.signatories="{ item }">
                <ol>
                  <li v-for="(e, i) in item.names" :key="i">
                    {{ e.name }} ({{ e.initials }})
                  </li>
                </ol>
              </template>
...
export default {
  ...
  methods: {
    ...
    createSignatory() {
      ...
      axios
        .post(route("signatories_settings.store"), this.formData)
        .then((res) => {
          this.submit = false;
    ...
    updateSignatory() {
      ...
      axios
        .put(route("signatories_settings.update", this.formData.id), this.formData)
        .then((res) => {
          this.submit = false;
          this.getSignatoriesSettings();
          this.resetForm();
    ...
    getSignatoriesSettings() {
      this.loading = true;
      axios
        .get(route("signatories_settings.index"))
        .then((res) => {
          this.loading = false;
    ...
    deleteSignatory(signatory) {
      if (confirm(`Are you sure you want to delete ?`)) {
        axios
          .delete(route("signatories_settings.destroy", signatory.id))
          .then((res) => {
